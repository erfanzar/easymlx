...
class NanochatAttention(nn.Module):
    ...
    def __init__(self, config: NanochatConfig):
        ...
        # Precompute negated RoPE frequencies
    ...
    def __call__(
        ...
        *,
        ...
        # Nanochat applies RoPE manually then QK norm after, so we cannot
        # use the AttentionPerformer's built-in rope. Instead we apply rope
        # and QK norm ourselves, then pass to the performer without rope.
        if q.ndim == 3:
            # Flat (paged) layout: (num_tokens, num_heads, head_dim)
            offset = cache_view.offset if cache_view is not None else 0
            q = q[:, None, :, :]  # add fake seq dim for rope
            k = k[:, None, :, :]
            ...
            # Standard layout: (batch, seq, num_heads, head_dim)
            offset = cache_view.offset if cache_view is not None else 0
            # transpose to (B, H, L, D) for rope
        ...
        # Use attention performer without rope (already applied)
...
class NanochatDecoderLayer(nn.Module):
    ...
    def __call__(
        ...
        *,
        ...
        """
        # Pre-norm with functional RMSNorm (no learnable parameters)
