...
"""InternLM3 configuration for EasyMLX inference."""
...
@register_config("internlm3")
class InternLM3Config(EasyMLXBaseConfig):
    """Configuration for the InternLM3 transformer model.

    Very similar to InternLM2 but uses separate qkv_bias and rope_type
    fields following the newer HuggingFace convention.

    Attributes:
        model_type: Identifier string (``"internlm3"``).
        ...
        bias: Whether MLP projections include bias.
        qkv_bias: Whether QKV projections include bias.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            rope_theta: RoPE base frequency.
            rope_traditional: Whether to use traditional RoPE layout.
            rope_scaling: Optional RoPE scaling config. Must contain
                ``"factor"`` and ``"rope_type"`` (``"linear"`` or ``"dynamic"``).
            bias: Whether MLP projections include bias.
        ...
        if rope_scaling is not None:
            ...
            if rope_scaling["rope_type"] not in ["linear", "dynamic"]:
                raise ValueError("rope_scaling 'rope_type' currently only supports 'linear' or 'dynamic'")
