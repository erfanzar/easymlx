...
# ---------------------------------------------------------------------------
# SSM utilities (from mlx-lm ssm.py)
# ---------------------------------------------------------------------------
...
class FalconH1DecoderLayer(nn.Module):
    ...
    def __call__(
        ...
        *,
        ...
        # Mamba block (no cache needed -- state is internal)
        mamba_h = self.mamba(h)

        # Attention block (uses standard EasyMLX cache)
...
class FalconH1Model(EasyMLXBaseModule):
    ...
    def sanitize(self, weights: dict[str, mx.array]) -> dict[str, mx.array]:
        ...
        """
        # Check if sanitization is needed by inspecting the conv1d weight shape.
        conv_key = None
        for k in weights:
