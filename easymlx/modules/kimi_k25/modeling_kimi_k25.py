...
class KimiK25Model(EasyMLXBaseModule):
    ...
    def sanitize(self, weights: dict[str, mx.array]) -> dict[str, mx.array]:
        ...
        # Handle language_model prefix: remap and delegate sanitization
        ...
        if lm_weights:
            # Separate lm_head from model weights
            model_prefix = "model."
            inner_weights = {}
            head_weights = {}
...
class KimiK25ForCausalLM(BaseCausalLMModule[KimiK25Model, KimiK25Config]):
    ...
    def sanitize(self, weights: dict[str, mx.array]) -> dict[str, mx.array]:
        ...
        """
        # First strip vision keys and remap at the base model level
        base_model = self.base_model
        if hasattr(base_model, "sanitize"):
            weights = base_model.sanitize(weights)
        # Then apply BaseCausalLMModule sanitization (rotary inv_freq, tied lm_head)
        return {
            key: value
            for key, value in weights.items()
            if "rotary_emb.inv_freq" not in key and "rope.inv_freq" not in key
