...
"""Colorized, lazy-initializing logging utilities for easymlx workers.

Provides :class:`eLogger`, a lazy logger that defers handler setup until
first use, :class:`ColorFormatter` for ANSI-colored console output, and
:class:`ProgressLogger` for terminal progress bars with ETA display.
"""
...
class ColorFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        """Format a log record with ANSI color codes and timestamps.

        Each line of a multi-line message receives a colored prefix
        containing the timestamp and logger name.
...
class eLogger:
    ...
    def __init__(self, name: str, level: int | None = None):
        ...
        Args:
            name: The logger name used for the underlying
                :class:`logging.Logger`.
            level: Numeric logging level. If ``None``, reads from the
                ``LOGGING_LEVEL_ED`` environment variable (default
                ``"INFO"``).
        """
        if level is None:
    ...
    @property
    def level(self):
        """int: The numeric logging level for this logger."""
        return self._level
    ...
    def _ensure_initialized(self) -> None:
        """Create and configure the underlying logger on first call.

        Sets up a :class:`logging.StreamHandler` with a
        :class:`ColorFormatter` and attaches it to a new
        :class:`logging.Logger`.  Subsequent calls are no-ops.
        """
    ...
    def __getattr__(self, name: str) -> tp.Callable:
        """Proxy logging method calls to the lazily initialized logger.

        Intercepts attribute access for standard logging methods
        (``debug``, ``info``, ``warning``, ``error``, ``critical``,
        ``exception``, ``log``) and returns a wrapper that triggers
        ...
        Returns:
            A callable that delegates to the underlying logger method.

        Raises:
            AttributeError: If ``name`` is not a recognized logging
                method.
        """
...
class ProgressLogger:
    ...
    def complete(self, message: str | None = None, show_time: bool = True) -> None:
        ...
        if self.use_tty:
            ...
            self._logger.info(f"{message}{time_str}")
