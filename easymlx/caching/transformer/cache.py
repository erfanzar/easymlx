...
class TransformerCache(BaseCache):
    ...
    @property
    def views(self) -> list[TransformerCacheView | None]:  # type: ignore["override"]
        """Return the ordered list of per-layer transformer cache views."""
        return self._views

    @classmethod
    def init_cache(cls, config: TransformerCacheConfig, **kwargs: Any) -> TransformerCache:  # type: ignore["override"]
