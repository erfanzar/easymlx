...
class ResponsesResponse(BaseModel):
    ...
    This follows the OpenAI Responses API format, supporting multi-turn
    conversations with response storage and tool calling.
    ...
    Attributes:
        ...
        previous_response_id: ID of the previous response in the conversation.
        store: Whether this response was stored for future retrieval.
...
class ResponsesRequest(BaseModel):
    ...
    Supports the OpenAI Responses API, including multi-turn conversations
    via ``previous_response_id`` or ``conversation``, system instructions,
    and reasoning content inclusion.
    ...
    Attributes:
        ...
        store: Whether to store this response for later retrieval.
        previous_response_id: ID of a stored response to continue from.
        conversation: Conversation identifier or configuration.
...
class ToolDefinition(BaseModel):
    """OpenAI-format tool definition for function calling.

    Attributes:
        type: Tool type, always ``"function"``.
        function: Dictionary containing the function name, description,
...
class ToolExecutionRequest(BaseModel):
    """Request body for the ``/v1/tools/execute`` endpoint.

    Supports multiple ways to specify the tool to execute: via a full
    ``ToolCall`` object, a ``FunctionCall``, or bare ``name``/``arguments``.

    Attributes:
        model: Optional model context for the tool execution.
        tool_call: Full tool call object (preferred).
        function: Function call object (alternative to ``tool_call``).
        name: Tool name (used when ``tool_call`` and ``function`` are absent).
        arguments: Tool arguments as JSON string, dict, or list.
...
class ToolExecutionResponse(BaseModel):
    """Response body for the ``/v1/tools/execute`` endpoint.

    Attributes:
        id: Unique execution identifier.
        model: Model context associated with the execution.
        tool_call_id: The ID of the originating tool call, if applicable.
        name: The name of the tool that was executed.
        arguments: Normalized arguments that were passed to the tool handler.
        output: The return value from the tool handler on success.
        error: Error details if the execution failed.
        status: Execution outcome (``"completed"``, ``"error"``,
            ``"not_implemented"``).
    """
