...
"""Qwen3 Coder tool parser module.

This module provides the Qwen3CoderToolParser class for parsing XML-style tool
calls generated by Qwen3 Coder models. The parser handles both complete and
streaming extraction of tool calls with automatic type conversion based on
schema definitions.
...
class Qwen3CoderToolParser(ToolParser):
    ...
    Handles XML-style tool calls with function names and parameter tags.
    Supports complex parameter value conversion based on type definitions.

    Features:
        - XML tag-based parsing (<tool_call>, <function=...>, <parameter=...>)
        - Automatic type conversion (string, int, float, bool, object)
        - Streaming with JSON fragment generation
        - Parameter value escaping for JSON compatibility
        - State tracking for multi-parameter functions
    ...
    Maintains extensive streaming state including current parameter
    tracking, JSON assembly, and completion detection.
    ...
    Attributes:
        ...
        tool_call_start_token: XML token marking tool call start.
        tool_call_end_token: XML token marking tool call end.
        tool_call_prefix: Prefix for function definition.
        function_end_token: Token marking function definition end.
        parameter_prefix: Prefix for parameter definition.
        parameter_end_token: Token marking parameter end.
        is_tool_call_started: Whether a tool call has started.
        current_tool_index: Index of the current tool being processed.
        current_function_name: Name of the current function being parsed.
        current_param_name: Name of the current parameter being parsed.
        current_param_value: Value being accumulated for current parameter.
    """

    def __init__(self, tokenizer: AnyTokenizer):
        """Initialize the Qwen3 Coder tool parser.

        Sets up the parser with XML tokens, regex patterns, and initializes
        streaming state for tool call extraction.

        Args:
            tokenizer: The tokenizer instance used for encoding/decoding tokens.
                Must have a vocabulary containing the tool call start and end tokens.

        Raises:
            ValueError: If the tokenizer is None or not provided.
            RuntimeError: If tool call start/end tokens are not found in the
        ...
        self.current_tool_string_id: str | None = None
    ...
    def _generate_tool_call_id(self) -> str:
        ...
        Returns:
            A string in the format "call_<24-char-hex>" where the hex portion
            is derived from a UUID4.
        """
    ...
    def _reset_streaming_state(self):
        ...
        for processing a new response. This includes tool indices, function
        names, parameter tracking, and JSON assembly state.
        """
    ...
    def _parse_xml_function_call(self, function_call_str: str, tools: list[ToolDefinition] | None) -> ToolCall | None:
        ...
        Extracts the function name and parameters from an XML-formatted function
        call string, applying type conversion based on the tool schema.

        Args:
            function_call_str: The XML string containing the function call,
                starting after the function= prefix and ending before </function>.
            tools: Optional list of tool definitions containing parameter schemas
                for type conversion.

        Returns:
            A ToolCall object with the parsed function name and JSON-encoded
        ...
        def convert_param_value(param_value: str, param_name: str, param_config: dict, func_name: str) -> Any:
            """Convert a parameter value to its appropriate type based on schema.

            Performs type conversion from string to the type specified in the
            parameter configuration (int, float, bool, object, etc.).

            Args:
                param_value: The string value to convert.
                param_name: The name of the parameter.
                param_config: Dictionary containing parameter type information.
                func_name: The function name (for logging purposes).

            Returns:
                The converted value in the appropriate type, or the original
                string if conversion fails or type is unknown.
            """
            ...
            if param_type in ["string", "str", "text", "varchar", "char", "enum"]:
                ...
                logger.warning(
                    "Parameter '%s' has unknown type '%s'. The value will be treated as a string.",
                    param_name,
                    param_type,
                )
                return param_value
    ...
    def _get_function_calls(self, model_output: str) -> list[str]:
        ...
        Parses the model output to find all function call definitions within
        tool_call tags.

        Args:
            model_output: The raw model output string containing tool calls.

        Returns:
            A list of function call strings, each containing the function
            definition (name and parameters) without the outer tags.
        """
    ...
    def extract_tool_calls(
        ...
        converting them to ToolCall objects with proper typing.
        ...
        Returns:
            ExtractedToolCallInformation containing:
                ...
                - content: Any text content before the tool calls
        """
    ...
    def extract_tool_calls_streaming(
        ...
        Processes incremental model output to extract tool calls as they are
        generated, maintaining state across calls to produce proper streaming
        deltas.
        ...
        Returns:
            A DeltaMessage containing either:
                - content: Regular text content if not in a tool call
                - tool_calls: Delta tool call information with function name
                  or argument fragments
