...
def make_reasoning_stripper(
    *,
    ...
    def strip_reasoning(text: str) -> str:
        """Remove reasoning blocks from model output text.

        Handles both closed tags (``start...end``) and unclosed tags where
        the model hit the token limit before producing the end tag.

        Args:
            text: Raw model output text potentially containing reasoning blocks.

        Returns:
            The text with all reasoning blocks removed and whitespace stripped.
        """
