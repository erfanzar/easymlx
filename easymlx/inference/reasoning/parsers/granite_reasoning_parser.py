...
class GraniteReasoningParser(ReasoningParser):
    ...
    def is_reasoning_end(self, input_ids: Sequence[int]) -> bool:
        ...
        Args:
            input_ids: Sequence of token IDs to decode and check.

        Returns:
            ``True`` if any response starter phrase is found in the decoded text.
        """
    ...
    def extract_content_ids(self, input_ids: list[int]) -> list[int]:
        """Return all token IDs unchanged.

        Granite uses text-level delimiter splitting rather than token-level
        extraction, so all token IDs are returned as-is.
    ...
    def extract_reasoning(self, model_output: str, request=None) -> tuple[str | None, str | None]:
        ...
        Returns:
            ...
            ``(None, full_output)`` if no thought delimiter is found.
        """
