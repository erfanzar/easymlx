...
class Step3p5ReasoningParser(Step3ReasoningParser):
    ...
    Step3.5 uses the same asymmetric token format as Step3 (only the
    ``</think>`` end token is generated by the model). All parsing logic
    is inherited from :class:`Step3ReasoningParser`.

    Example:
        >>> parser = Step3p5ReasoningParser()
