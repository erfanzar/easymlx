...
class DeepSeekV3ReasoningParser(ReasoningParser):
    ...
    def _sync_delegate_state(self) -> None:
        """Synchronize manual override flags from this wrapper to the delegate.

        Ensures that if ``assume_reasoning`` is set on the wrapper instance,
        the delegate parser also receives the flag.
        """
    ...
    def configure_prompt_context(self, prompt_text: str, prompt_token_ids: Sequence[int]) -> None:
        ...
        If the current delegate is ``IdentityReasoningParser`` but the prompt
        ends with the R1 start token (``<think>``), the delegate is upgraded
        to ``DeepSeekR1ReasoningParser`` to handle the reasoning content.

        Args:
            prompt_text: Prompt text that is sent to the model.
            prompt_token_ids: Token IDs for the prompt text.
        """
    ...
    def is_reasoning_end(self, input_ids: Sequence[int]) -> bool:
        ...
        Returns:
            Result from the delegate's ``is_reasoning_end`` method.
        """
        self._sync_delegate_state()
    ...
    def extract_content_ids(self, input_ids: list[int]) -> list[int]:
        ...
        Returns:
            Content-only token IDs from the delegate parser.
        """
        self._sync_delegate_state()
    ...
    def extract_reasoning(self, model_output: str, request=None) -> tuple[str | None, str | None]:
        ...
        Returns:
            Tuple of ``(reasoning_content, visible_content)`` from the delegate.
        """
        self._sync_delegate_state()
    ...
    def extract_reasoning_streaming(
        ...
        Returns:
            DeltaMessage from the delegate, or ``None``.
        """
        self._sync_delegate_state()
