...
type ReasoningParserName = Literal[
    "deepseek_r1",
    "deepseek-r1",
    "deepseek_v3",
    "glm45",
...
class ReasoningParserManager:
    ...
    def get_reasoning_parser(cls, name: str) -> type:
        """Retrieve a registered reasoning parser class by name.

        Args:
            name: The registered name of the parser (e.g. ``"deepseek_r1"``).

        Returns:
            The parser class (subclass of ``ReasoningParser``).
    ...
    @classmethod
    def available_parsers(cls) -> tuple[str, ...]:
        """Return a sorted tuple of all registered parser names.

        Returns:
            A tuple of registered parser name strings in alphabetical order.
        """
        return tuple(sorted(cls.reasoning_parsers))

    @classmethod
    def _register_module(cls, module: type, module_name: str | list[str] | None = None, force: bool = True) -> None:
        """Internal method to register a parser class under one or more names.

        Args:
            module: The parser class to register (must be a ``ReasoningParser``
                subclass).
            module_name: Name(s) to register the parser under. Defaults to the
                class ``__name__``.
            force: If ``True``, overwrite existing registrations. If ``False``,
                raise ``KeyError`` on name conflicts.

        Raises:
            TypeError: If *module* is not a ``ReasoningParser`` subclass.
            KeyError: If *force* is ``False`` and *module_name* is already
                registered.
        """
    ...
    def register_module(
        ...
        """Register a reasoning parser module.

        Can be used as a decorator or called directly with the *module* argument.

        Args:
            name: Name(s) to register the parser under. Defaults to the class
                ``__name__`` if ``None``.
            force: If ``True`` (default), overwrite existing registrations.
            module: The parser class to register directly. If ``None``, returns
                a decorator that registers the decorated class.

        Returns:
            The registered class if *module* is provided, or a decorator
            function if *module* is ``None``.

        Raises:
            TypeError: If *force* is not a bool, or *name* is not a valid type.
        """
    ...
    def import_reasoning_parser(cls, plugin_path: str) -> None:
        ...
        The file is expected to contain a ``ReasoningParser`` subclass decorated
        with ``@ReasoningParserManager.register_module``. Import errors are
        logged as warnings rather than raised.

        Args:
            plugin_path: Absolute or relative file path to the Python file
                containing the parser implementation.
        """
...
def _import_from_path(module_name: str, file_path: str | os.PathLike):
    ...
    Args:
        module_name: The module name to register in ``sys.modules``.
        file_path: The file system path to the Python source file.

    Returns:
        The imported module object.

    Raises:
        ModuleNotFoundError: If no module spec could be created from the path.
    """
...
def _is_list_of(value: object, typ, *, check: Literal["first", "all"] = "first") -> bool:
    """Check whether a value is a list whose elements are instances of a type.

    Args:
        value: The value to check.
        typ: The expected element type.
        check: Strategy for checking elements. ``"first"`` checks only the
            first element for efficiency. ``"all"`` checks every element.

    Returns:
        ``True`` if *value* is a list and its elements match *typ* according
        to the *check* strategy. An empty list returns ``True``.
    """
