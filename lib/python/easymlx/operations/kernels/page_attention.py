...
def _causal_paged_attention_reference(
    query: mx.array,
    key_cache: mx.array,
    ...
    *,
    ...
    """Exact causal fallback for one multi-token paged query sequence."""
    qsl = mx.array(query_start_loc, dtype=mx.int32)
    if int(qsl.shape[0]) != 2:
        return None
    query_len = int((qsl[1] - qsl[0]).item())
    if query_len <= 1 or int(query.shape[0]) != query_len:
        return None

    kv_lens_arr = mx.array(kv_lens, dtype=mx.int32)
    if int(kv_lens_arr.shape[0]) != 1:
        return None
    kv_len = int(kv_lens_arr[0].item())
    past_len = kv_len - query_len
    if kv_len <= 0 or past_len < 0:
        return None

    tables = mx.array(block_tables, dtype=mx.int32)
    num_blocks = (kv_len + int(block_size) - 1) // int(block_size)
    block_ids = tables[0, :num_blocks]
    keys = key_cache[block_ids].reshape((-1, key_cache.shape[-2], key_cache.shape[-1]))[:kv_len]
    values = value_cache[block_ids].reshape((-1, value_cache.shape[-2], value_cache.shape[-1]))[:kv_len]

    if int(query.shape[1]) != int(keys.shape[1]):
        repeats = int(query.shape[1]) // int(keys.shape[1])
        keys = mx.repeat(keys, repeats, axis=1)
        values = mx.repeat(values, repeats, axis=1)

    scale = float(softmax_scale) if softmax_scale is not None else float(query.shape[-1]) ** -0.5
    scores = mx.einsum("qhd,khd->hqk", query.astype(mx.float32), keys.astype(mx.float32)) * scale
    key_positions = mx.arange(kv_len, dtype=mx.int32)
    max_positions = past_len + mx.arange(query_len, dtype=mx.int32)
    mask = key_positions[None, :] <= max_positions[:, None]
    if sliding_window:
        min_positions = mx.maximum(max_positions - int(sliding_window) + 1, 0)
        mask = mask & (key_positions[None, :] >= min_positions[:, None])
    scores = mx.where(mask[None, :, :], scores, mx.array(-1.0e9, dtype=scores.dtype))
    probs = mx.softmax(scores, axis=-1)
    return mx.einsum("hqk,khd->qhd", probs.astype(values.dtype), values).astype(query.dtype)
...
class PageAttention(BaseOperation):
    ...
    def forward_native(
        ...
        **_: tp.Any,
        ...
        if not bool(cache_metadata.is_single_token_decode):
            reference_outputs = _causal_paged_attention_reference(
                query,
                runtime_key_cache,
                runtime_value_cache,
                block_tables=cache_metadata.block_tables,
            ...
            if reference_outputs is not None:
                return AttentionOutput(attention_outputs=reference_outputs, cache_view=cache_view)
