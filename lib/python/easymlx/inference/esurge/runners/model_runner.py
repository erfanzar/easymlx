...
class _CompiledPagedCacheProxy:
    ...
    def __init__(self, cache: Any):
        ...
        self.cache_dtype_is_fp8 = bool(getattr(cache, "cache_dtype_is_fp8", False))
        self.k_scales = getattr(cache, "k_scales", None)
        self.v_scales = getattr(cache, "v_scales", None)
        self.state = {
    ...
    @staticmethod
    def _cast_like_cache(x: mx.array, cache: mx.array) -> mx.array:
    ...
    def concatenate_to_cache(
        ...
        *,
        ...
        if query_lens is None:
            query_lens = (1,) * len(resolved_slot_ids)
        if not resolved_slot_ids or any(int(query_len) != 1 for query_len in query_lens):
            raise ValueError("Compiled paged cache proxy only supports single-token decode.")

        slot_array = mx.array(list(resolved_slot_ids), dtype=mx.int32)
        token_positions = self.kv_lens[slot_array].astype(mx.int32)
        ...
        if rope is not None:
            ...
            prepared_queries = rope(q_batched, offset=token_positions).transpose(0, 2, 1, 3)[:, 0]
            prepared_keys = rope(k_batched, offset=token_positions).transpose(0, 2, 1, 3)[:, 0]

        block_ids = self.block_tables[slot_array, token_positions // self.block_size]
        in_block_offsets = token_positions % self.block_size
        self.key_cache[block_ids, in_block_offsets] = self._cast_like_cache(prepared_keys, self.key_cache)
        self.value_cache[block_ids, in_block_offsets] = self._cast_like_cache(prepared_values, self.value_cache)

        if self.page_key_cache is not None and self.page_value_cache is not None and self.page_vec_size > 0:
            num_vecs = self.head_dim // self.page_vec_size
            page_keys = self._cast_like_cache(prepared_keys, self.page_key_cache)
            page_values = self._cast_like_cache(prepared_values, self.page_value_cache)
            packed_keys = page_keys.reshape(-1, self.num_kv_heads, num_vecs, self.page_vec_size)
            self.page_key_cache[block_ids, :, :, in_block_offsets] = packed_keys
            self.page_value_cache[block_ids, :, :, in_block_offsets] = page_values
...
def _as_mx_logits(output: Any, *, promote_to_float32: bool = True) -> mx.array:
    ...
    if promote_to_float32 and logits.dtype != mx.float32:
...
def _argmax_token_ids(output: Any) -> list[int]:
    ...
    """
    token_ids = mx.argmax(_as_mx_logits(output, promote_to_float32=False), axis=-1).astype(mx.int64)
...
def _strict_eval_compiled_outputs(logits: Any) -> None:
    logits = _as_mx_logits(logits, promote_to_float32=False)
    token_ids = mx.argmax(logits, axis=-1).astype(mx.int32)
    mx.eval(token_ids)
...
class ModelRunner:
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            min_input_pad: Minimum request-count bucket used for
                compilation/warmup padding.
            min_token_pad: Optional minimum token-count bucket used for
                compilation/warmup padding.
        ...
        self._decode_step_with_state_fn = getattr(self.model, "decode_step_with_state", None)
        ...
        self._compiled_forwards: dict[tuple[str, tuple[int, ...], tuple[int, ...]], dict[str, Any]] = {}
        self._compiled_samplers: dict[tuple[tuple[int, ...], bool, float, int, float], Any] = {}
        self._compiled_batch_padders: dict[tuple[int, int], Any] = {}
        self._failed_compiled_forward_keys: set[tuple[str, tuple[int, ...], tuple[int, ...]]] = set()
    ...
    def _get_token_buckets(self) -> list[int]:
        ...
        min_bucket = min(self.min_token_pad or 1, cap)
        buckets = [min_bucket]
        v = 1
        while v < min_bucket:
            v *= 2
    ...
    def precompile(self) -> None:
        ...
        for idx, (num_tokens, num_reqs) in enumerate(pairs):
            ...
            compiled_entry = None
            try:
                compiled_entry = self._get_compiled_forward(
                    slot_ids,
                    query_lens,
                    prefer_decode_step=self._can_use_decode_step(query_lens, has_extra=False),
                )
                if compiled_entry is not None:
                    raw_output = self._execute_compiled_forward(compiled_entry, dummy_ids)
                ...
                uses_decode_state = bool(compiled_entry and compiled_entry.get("use_decode_state"))
                compile_key = (
                    "decode_state"
                    if uses_decode_state
                    else "decode_step"
                    if self._can_use_decode_step(query_lens, has_extra=False)
                    else "forward",
    ...
    def _get_compiled_forward(
        ...
        *,
        ...
        if not query_lens or any(int(query_len) != 1 for query_len in query_lens):
            return None
        use_decode_state = bool(
            prefer_decode_step
            and (len(slot_ids) == 1 or bool(getattr(self.model, "supports_multitoken_decode_state", False)))
            and callable(self._decode_step_with_state_fn)
            and self._can_use_decode_step(query_lens, has_extra=False)
        )
        if self._compiled_forward_blocked and not use_decode_state:
            return None
        use_decode_step = bool(
            not use_decode_state and prefer_decode_step and self._can_use_decode_step(query_lens, has_extra=False)
        )
        key = (
            "decode_state" if use_decode_state else "decode_step" if use_decode_step else "forward",
            tuple(int(slot_id) for slot_id in slot_ids),
            tuple(int(query_len) for query_len in query_lens),
        )
        ...
        if use_decode_state or use_decode_step:
            static_kwargs = {
                "cache_views": cache_views,
                "cache_metadata": cache_metadata,
            }
            if use_decode_step and not self._decode_step_accepts_kwargs:
                static_kwargs = {
                    name: value for name, value in static_kwargs.items() if name in self._decode_step_param_names
                }
        else:
        ...
        decode_state_box = {"decode_state": self._get_model_decode_state()} if use_decode_state else None

        def compiled_forward(input_ids: mx.array) -> Any:
            if use_decode_state:
                logits, new_decode_state = self._decode_step_with_state_fn(
                    input_ids=input_ids,
                    decode_state=decode_state_box["decode_state"],
                    **static_kwargs,
                )
                decode_state_box["decode_state"] = new_decode_state
                return logits
            if use_decode_step:
                output = self._decode_step_fn(input_ids=input_ids, **static_kwargs)
            else:
                output = self.model(input_ids=input_ids, **static_kwargs)
            return getattr(output, "logits", output)

        compile_state = [state_arrays, decode_state_box] if use_decode_state else state_arrays
        compiled_fn = mx.compile(compiled_forward, inputs=compile_state, outputs=compile_state)
        compiled_entry = {
            "fn": compiled_fn,
            "state": state_arrays,
            "proxies": cache_proxies,
            "use_decode_state": use_decode_state,
            "decode_state_box": decode_state_box,
            "key": key,
            "validated": False,
        }
    ...
    def _get_model_decode_state(self) -> Any:
        get_state = getattr(self.model, "get_decode_state", None)
        if not callable(get_state):
            raise RuntimeError("Model does not expose get_decode_state().")
        return get_state()

    def _set_model_decode_state(self, state: Any) -> None:
        set_state = getattr(self.model, "set_decode_state", None)
        if not callable(set_state):
            raise RuntimeError("Model does not expose set_decode_state().")
        set_state(state)

    def _execute_compiled_forward(self, compiled_entry: dict[str, Any], input_ids: mx.array) -> dict[str, mx.array]:
        self._refresh_compiled_cache_state(compiled_entry, self.kv_caches)
        compiled_fn = compiled_entry["fn"]
        if compiled_entry.get("use_decode_state"):
            decode_state_box = compiled_entry["decode_state_box"]
            decode_state_box["decode_state"] = self._get_model_decode_state()
            logits = compiled_fn(input_ids)
            if not compiled_entry.get("validated", False):
                _strict_eval_compiled_outputs(logits)
                compiled_entry["validated"] = True
            self._sync_compiled_cache_state(compiled_entry, self.kv_caches)
            self._set_model_decode_state(decode_state_box["decode_state"])
    ...
    def _init_seq_buckets(
        ...
        if user_buckets:
            ...
            buckets = self._get_request_paddings(self.min_input_pad, max_num_seqs)
    ...
    @staticmethod
    def _copy_cache_row(table: Any, row_index: int) -> Any:
        """Return a detached copy of one cache-table row."""
        if isinstance(table, mx.array):
            return mx.take(table, mx.array([int(row_index)], dtype=mx.int32), axis=0)[0]
        row = table[int(row_index)]
        copy = getattr(row, "copy", None)
        return copy() if callable(copy) else row

    @staticmethod
    def _zero_indexed_cache_rows(array: Any | None, indices: mx.array) -> mx.array | None:
        """Zero selected cache rows using a payload with the cache dtype."""
        if array is None or indices.size == 0:
            return None
        if not isinstance(array, mx.array):
            ...
            try:
                ...
                for index in index_list:
                    array[index] = 0
            return None
        row_shape = (int(indices.shape[0]), *tuple(int(dim) for dim in array.shape[1:]))
        array[indices] = mx.zeros(row_shape, dtype=array.dtype)
        return array
    ...
    def _zero_pages(self, page_ids: list["int"]) -> None:
        ...
        for cache in self.kv_caches:
            ...
            for array in (
                key_cache,
                value_cache,
                getattr(cache, "page_key_cache", None),
                getattr(cache, "page_value_cache", None),
            ):
                touched = self._zero_indexed_cache_rows(array, filtered)
                if touched is not None:
                    touched_arrays.append(touched)
            for attr_name in ("key_norms", "key_residual_norms", "key_qjl_signs", "value_norms"):
                aux_array = getattr(cache, attr_name, None)
                touched = self._zero_indexed_cache_rows(aux_array, filtered)
                if touched is not None:
                    touched_arrays.append(touched)
        if touched_arrays:
    ...
    def _assign_row_mapping(
        ...
        *,
        ...
        The valid KV range is controlled by ``kv_lens``. Freshly assigned
        pages are overwritten during prefill/decode, so binding must avoid
        synchronously clearing every page in the hot path.
            ...
            if self.kv_caches:
                ...
                if use_decode_step:
                    compiled_entry = self._get_compiled_forward(
                        slot_ids,
                        query_lens,
                        prefer_decode_step=True,
                    )
                    if compiled_entry is not None:
                        try:
                            return self._execute_compiled_forward(compiled_entry, batch_token_ids)
                        except (RuntimeError, ValueError):
                            key = compiled_entry.get(
                                "key",
                                (
                                    "decode_step",
                                    tuple(int(slot_id) for slot_id in slot_ids),
                            ...
                            logger.debug("Compiled decode_step failed, falling back to eager")
                    return self._call_decode_step(
    ...
    def _forward_step(self, request: ExecutionRequest) -> tuple[Any, mx.array]:
        ...
        return raw_output, _as_mx_logits(raw_output, promote_to_float32=False)
    ...
    def _sample_next_tokens_mx(
        ...
        *,
        ...
        needs_float32_logits = False
        for params in sampling_params_by_row:
            if params is None:
                continue
            repetition_penalty = float(getattr(params, "repetition_penalty", 1.0) or 1.0)
            presence_penalty = float(getattr(params, "presence_penalty", 0.0) or 0.0)
            if bool(params.do_sample) or repetition_penalty != 1.0 or presence_penalty != 0.0:
                needs_float32_logits = True
                break
        if needs_float32_logits and logits.dtype != mx.float32:
            logits = logits.astype(mx.float32)
        ...
        for (do_sample, temperature, top_k, top_p), row_indices in grouped_rows.items():
            if len(row_indices) == len(sampling_params_by_row) and all(
                int(row_index) == offset for offset, row_index in enumerate(row_indices)
            ):
                group_logits = logits
            else:
            ...
            if not do_sample:
                group_tokens = mx.argmax(group_logits, axis=-1).astype(mx.int32)
    ...
    def run(self, request: ExecutionRequest) -> ExecutionResult:
        ...
        logits = _as_mx_logits(raw_output, promote_to_float32=False)
