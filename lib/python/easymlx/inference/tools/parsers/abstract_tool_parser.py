...
while EasyMLX keeps it under `easymlx.inference.tools.abstract_tool`. This module
exists so code ported from vLLM (or referencing that layout) can import from
`easymlx.inference.tools.parsers.abstract_tool_parser` without needing to know
EasyMLX's internal path.
...
Example:
    >>> from easymlx.inference.tools.parsers.abstract_tool_parser import (
    ...     ToolParser,
    ...     ToolParserManager
    ... )
...
Note:
    ...
    importing directly from `easymlx.inference.tools.abstract_tool`.
"""
