...
ported from vLLM to work seamlessly with EasyMLX's internal structure.

vLLM keeps tool parsing helpers under `tool_parsers/utils.py`, while EasyMLX
keeps them under `easymlx.inference.tools.utils`. This module re-exports
EasyMLX's utilities to maintain API compatibility with vLLM's layout.
...
Example:
    >>> from easymlx.inference.tools.parsers.utils import is_complete_json
...
Note:
    ...
    importing directly from `easymlx.inference.tools.utils`.
"""
