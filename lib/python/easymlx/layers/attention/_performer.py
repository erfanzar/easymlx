...
def resolve_paged_attention_mechanism(attn_mechanism: object | None) -> str | None:
    """Resolve config attention aliases to a paged attention runtime name."""
    mechanism = canonical_attention_mechanism(attn_mechanism)
    if mechanism in {"unified_attention", "page_attention"}:
        return mechanism
    return None
...
def prepare_paged_attention_inputs(
    ...
    *,
    ...
    query_lens = None
    if getattr(cache_metadata, "is_single_token_decode", False) and resolved_slots:
        query_lens = (1,) * len(resolved_slots)
...
class AttentionPerformer:
        ...
        *,
        ...
        Args:
            ...
            attn_mechanism: Paged attention mechanism override. If ``None``,
                uses the module-level default (unified_attention).
                Legacy ``"paged"`` and ``"auto"`` values resolve to
                ``"unified_attention"``. Pass ``"page_attention"`` to
                explicitly use the old page_attention kernels.
        """
        self.scale = float(scale)
        self.attn_mechanism = resolve_paged_attention_mechanism(attn_mechanism)
    ...
    def __call__(
        ...
        *,
        ...
        """
        if isinstance(cache_view, PageCacheView) or callable(getattr(cache_view, "concatenate_to_cache", None)):
    ...
    def forward(
        ...
        *,
        ...
        if self.attn_mechanism is not None:
            paged_mechanism_kwargs["paged_attention_mechanism"] = self.attn_mechanism
