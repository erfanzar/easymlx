...
"""Bailing MoE configuration for serving and inference.

This module defines the configuration class for the Bailing MoE model,
registered with the EasyMLX factory under the ``"bailing_moe"`` model type.
"""
...
@register_config("bailing_moe")
class BailingMoeConfig(EasyMLXBaseConfig):
    """Configuration for the Bailing Mixture-of-Experts language model.

    Registered with the EasyMLX factory under the ``"bailing_moe"`` model type.

    Attributes:
        model_type: The model type identifier (``"bailing_moe"``).
        ...
        num_shared_experts: Number of shared (always-on) experts.
        num_experts_per_tok: Number of experts activated per token.
        first_k_dense_replace: Layers before this index use dense MLP.
        norm_topk_prob: Whether to normalize top-k routing probabilities.
        use_qk_norm: Whether to apply QK normalization.
        use_qkv_bias: Whether to use bias in QKV projection.
        use_bias: Whether to use bias in other linear projections.
        norm_head: Whether to normalize lm_head weights.
        score_function: Scoring function for router (``"softmax"`` or ``"sigmoid"``).
        n_group: Number of expert groups.
        topk_group: Number of groups to keep per token.
        routed_scaling_factor: Scaling factor for routed expert weights.
        moe_router_enable_expert_bias: Whether router uses expert bias.
        moe_router_enable_routed_scaling: Whether to scale routed outputs.
        moe_router_enable_shared_expert: Whether shared experts are enabled.
        moe_shared_expert_intermediate_size: Shared expert intermediate size override.
        partial_rotary_factor: Fraction of dims for RoPE.
        rotary_dim: Explicit rotary embedding dimension.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """Initialize the Bailing MoE configuration.
