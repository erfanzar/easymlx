...
"""Qwen2 MoE MLX model implementation for serving and inference.

Structure mirrors EasyDeL's qwen2_moe:
  Qwen2MoeConfig -> Qwen2MoeAttention -> Qwen2MoeMLP -> Qwen2MoeSparseBlock
...
class Qwen2MoeAttention(nn.Module):
    """Multi-head attention with grouped-query attention for Qwen2-MoE.

    Supports configurable QKV bias, rotary positional embeddings, and
    ...
    def __init__(self, config: Qwen2MoeConfig):
        """Initialize the Qwen2-MoE attention module.
...
class Qwen2MoeMLP(nn.Module):
    """SiLU-gated feed-forward MLP for Qwen2-MoE.
    ...
    def __init__(self, config: Qwen2MoeConfig, intermediate_size: int | None = None):
        """Initialize the MLP.
...
class Qwen2MoeSparseBlock(nn.Module):
    """Mixture-of-Experts block with a shared expert for Qwen2-MoE.

    Routes tokens through top-k routed experts and adds a gated shared
    expert output.
    ...
    def __call__(self, hidden_states: mx.array) -> mx.array:
        """Route tokens through experts and combine with shared expert output.
...
class Qwen2MoeDecoderLayer(nn.Module):
    """Single decoder layer for Qwen2-MoE.
    ...
    def __init__(self, config: Qwen2MoeConfig, layer_idx: int, *, use_sliding: bool = False):
        ...
        self.mlp = Qwen2MoeSparseBlock(config) if use_moe else Qwen2MoeMLP(config)
...
@register_module(task_type=TaskType.BASE_MODULE, config=Qwen2MoeConfig, model_type="qwen2_moe")
class Qwen2MoeModel(EasyMLXBaseModule):
    """Base Qwen2-MoE transformer model.

    Combines token embeddings, a stack of MoE/dense decoder layers with
    optional sliding-window attention, and final RMS normalization.

    Attributes:
        config_class: The associated configuration class (``Qwen2MoeConfig``).
    ...
    def __init__(self, config: Qwen2MoeConfig):
        """Initialize the base Qwen2-MoE model.
        ...
        self.layers = [
            Qwen2MoeDecoderLayer(config, idx, use_sliding=layer_type == "sliding_attention")
    ...
    def __call__(
        ...
        *,
        ...
        """Run the Qwen2-MoE transformer stack.
...
@register_module(task_type=TaskType.CAUSAL_LM, config=Qwen2MoeConfig, model_type="qwen2_moe")
class Qwen2MoeForCausalLM(BaseCausalLMModule[Qwen2MoeModel, Qwen2MoeConfig]):
    ...
    Wraps ``Qwen2MoeModel`` and adds vocabulary projection to produce
    next-token logits.
...
__all__ = ("Qwen2MoeForCausalLM", "Qwen2MoeModel")
