        ...
        **kwargs: Unpack[FromPretrainedKwargs],
        ...
        if subfolder is None:
            resolved = resolve_hf_composite_repo(
                ...
                task_type=cls.model_task,
            )

        if (
            cls.model_task == TaskType.BASE_MODULE
            and subfolder is None
            and resolved is not None
            and resolved.composite_model_type is not None
        ):
            registration = registry.get_module_registration(cls.model_task, resolved.composite_model_type)
        ...
        if subfolder is None and resolved is not None:
            subfolder = resolved.model_subfolder
        ...
        if cls.model_task == TaskType.CAUSAL_LM and model_type == "qwen3_5" and hasattr(hf_config, "text_config"):
