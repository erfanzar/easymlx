...
def _reshape_and_cache_metal(
    ...
    *,
    ...
    if key.dtype != page_key_cache.dtype:
        key = key.astype(page_key_cache.dtype)
    if value.dtype != page_value_cache.dtype:
        value = value.astype(page_value_cache.dtype)
...
class PageCacheView:
    ...
    @staticmethod
    def _cast_like_cache(array: mx.array, cache_array: mx.array) -> mx.array:
        """Cast *array* to the destination cache dtype when needed."""
        return array.astype(cache_array.dtype) if array.dtype != cache_array.dtype else array
    ...
    def append(self, seq_idx: int, key: mx.array, value: mx.array) -> None:
        ...
        for i in range(num_tokens):
            ...
            self.key_cache[block, offset] = self._cast_like_cache(key[i], self.key_cache)
            self.value_cache[block, offset] = self._cast_like_cache(value[i], self.value_cache)
            if self.page_key_cache is not None and self.page_value_cache is not None:
                page_key = self._cast_like_cache(key[i], self.page_key_cache)
                page_value = self._cast_like_cache(value[i], self.page_value_cache)
                packed_key = page_key.reshape(self.num_kv_heads, self.head_dim // self.page_vec_size, self.page_vec_size)
                self.page_key_cache[block, :, :, offset, :] = packed_key
                self.page_value_cache[block, :, :, offset] = page_value
        ...
        *,
        ...
        if is_single_token_decode:
            ...
            if self.cache_dtype_is_fp8 and self.k_scales is not None and self.v_scales is not None:
                ...
                self.value_cache[block_ids, in_block_offsets] = self._cast_like_cache(
                    prepared_values,
                    self.value_cache,
                )
            ...
            if page_key_cache is not None and page_value_cache is not None and page_vec_size > 0:
                ...
                page_keys = self._cast_like_cache(prepared_keys, page_key_cache)
                page_values = self._cast_like_cache(prepared_values, page_value_cache)
                packed_keys = page_keys.reshape(-1, self.num_kv_heads, num_vecs, page_vec_size)
        ...
        if slot_parts:
            ...
            if page_key_cache is not None and page_value_cache is not None and page_vec_size > 0:
                slot_mapping = block_ids * block_size + in_block_offsets
                page_keys_for_metal = self._cast_like_cache(prepared_keys, page_key_cache)
                page_values_for_metal = self._cast_like_cache(prepared_values, page_value_cache)
