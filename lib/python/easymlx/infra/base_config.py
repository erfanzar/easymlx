...
Typical usage::
    ...
    config = EasyMLXBaseConfig(attn_mechanism="unified", dtype="bfloat16")
...
class EasyMLXBaseConfig(PretrainedConfig):
    ...
    Attributes:
        ...
        attn_mechanism: The attention implementation to use. One of
            ``"auto"``, ``"vanilla"``, ``"sdpa"``, ``"paged"``,
            ``"unified"``, ``"unified_attention"``, ``"page_attention"``,
            or ``"paged_attention"``.
    ...
    Example::
        ...
        config = EasyMLXBaseConfig(attn_mechanism="unified", dtype="float16")
    ...
    def _validate(self) -> None:
        ...
        Raises:
            EasyMLXConfigError: If ``attn_mechanism`` is not one of the
                supported values, or if ``cache_dtype`` is not recognized.
        """
        if self.attn_mechanism not in (
            "auto",
            "vanilla",
            "sdpa",
            "paged",
