...
| `/v1/responses` | POST | Responses API for multi-turn conversations |
| `/v1/tools` | GET | List registered tools |
| `/v1/tools/execute` | POST | Execute a registered tool |
...
Enable response storage for multi-turn conversations via the Responses API:

```python
server = eSurgeApiServer(
    engines={"qwen3-0.6b": engine},
    enable_response_store=True,
    max_stored_responses=10_000,
    response_store_dir="/path/to/store",  # None for in-memory
)
```
