...
class TestCohere2:
    ...
    def test_registry(self):
        base_registration = registry.get_module_registration(TaskType.BASE_MODULE, "cohere2")
        lm_registration = registry.get_module_registration(TaskType.CAUSAL_LM, "cohere2")

        assert base_registration.module is Cohere2Model
        assert base_registration.config is Cohere2Config
        assert lm_registration.module is Cohere2ForCausalLM
        assert lm_registration.config is Cohere2Config
    ...
    def test_causal_lm(self, cohere2_config, small_model_config):
        ...
        assert result.success, f"Cohere2 CAUSAL_LM failed: {result.error_message}"
    ...
    def test_sanitize_matches_local_parameter_tree(self, cohere2_config):
        model = Cohere2ForCausalLM(cohere2_config)
