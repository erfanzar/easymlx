...
class TestGlm4V:
    """Test suite for GLM-4V model."""

    @pytest.fixture
    def glm4v_config(self, small_model_config):
        """Create GLM-4V-specific config."""
    ...
    def test_text_only(self, glm4v_config, small_model_config):
        """Test GLM-4V text-only forward pass."""
        ...
        assert result.success, f"GLM-4V text-only failed: {result.error_message}"

    def test_vision_language(self, glm4v_config, small_model_config):
        """Test GLM-4V vision-language forward pass."""
        model = Glm4VForConditionalGeneration(glm4v_config)
        num_image_tokens, image_grid_thw, pixel_values = _build_glm4v_vlm_inputs(glm4v_config)

        batch_size = 1
        vocab_size = small_model_config["vocab_size"]
        image_token_id = glm4v_config.image_token_id
    ...
    def test_generation(self, glm4v_config, small_model_config):
        """Test GLM-4V text-only generation."""
        tester = CausalLMTester()
        ...
        assert result.success, f"GLM-4V generation failed: {result.error_message}"
